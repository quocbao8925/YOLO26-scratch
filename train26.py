...
@torch.no_grad()
def validate(model, dataloader, device, nc, conf_thresh=0.25, iou_thresh=0.45):
    """Run validation, return mAP50."""
    model.eval()
    ...
    for batch in dataloader:
        ...
        y, _ = model(imgs)  # inference returns (y, preds)
    ...
    for c in unique_classes:
        ...
        ap50.append(np.trapz(np.interp(x, mrec, mpre), x))
...
def train(args):
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # Validation
        mAP50 = validate(ema.ema, val_loader, device, args.nc)
        fitness = mAP50
        current_lr = optimizer.param_groups[0]["lr"]
        print(f"  Epoch {epoch+1} complete — mAP50={mAP50:.4f}  lr={current_lr:.6f}")
